...
import UIBattleClock from "../Comn/UIBattleClock";

const {ccclass, property} = cc._decorator;

@ccclass
export default class UIItemRoleBattle_Landlord extends UIItemRoleBattleTS
{
    // //基础功能
    ...
    //地主标记
    @property({type:cc.Node})
    private conMarkLord:cc.Node = null;
    ...
    //加倍、不加倍
    @property({type:require('UIToggle')})
    private conActMulti:cc.Component = null;
    //不出
    @property({type:cc.Node})
    private conActDiscard:cc.Node = null;

    //倒计时//////////////////////////////
    @property({type:UIBattleClock})
    private uiCDClock:UIBattleClock = null;
    ...
    public OnInit(){
        ...
        if(this.conLightCard)
        this.conLightCard.active = false;
    }
    public OnReset(){
        // 托管中标记
        super.OnReset();
        ...
        this.conLightCard.active = false;
        //地主、农民标记
        this.conMarkLord.active = false;
        //倒计时闹钟标记
        this.uiCDClock.Hide();

        this.conActMulti.node.active = false;
    }
    ...
    // //new 4 douodizhu
    // public SetReady(isLightCard:boolean) {
    //     this.conReady.active = true;
    //     if(this.conLightCard)
    //         this.conLightCard.active = isLightCard;
    // }
    // public ClearReady (){this.conReady.active = false;}
    // public SetALord(multi:number) {}
    // public SetLightCard(multi:number) {}
    // public SetChipinMulti(multi:number) {}

    ////////////////////////////////////////////////////////////////////////
    public SetOpen (isLightCard:boolean) {
        if(this.conLightCard)
        this.conLightCard.active = isLightCard;
    }

    public CallLord(multi:number) {
        this.conActCallLord.active = true;
    }
    public SetLightCard(multi:number) {
        //TODO 需要明牌X几
        this.conActOpenCard.active = true;
    }
    public SetChipinMulti(multi:number) {
        this.conActMulti.node.active = true;
        this.conActMulti.SetChecked (multi > 0);
    }

    public MarkLord(multi:number) {
        this.conMarkLord.active = true;
        //multi: 叫地主时的倍数
    }

    //倒计时
    public SetCD (cd:number) {
        this.uiCDClock.Show(Math.ceil(cd / 1000));
    }


    //清理阶段性的提示、
    public ClearPhaseObjs () {
        this.conActOpenCard.active = false;
        this.conActCallLord.active = false;
        this.conActMulti.node.active = false;
        this.uiCDClock.Hide();
    }
}